...
class IsoTp:
    ...
    def __init__(self, arb_id_request, arb_id_response, bus, padding_value=0x00):
        self.bus = bus
    ...
    def send_message(self, data, arbitration_id, force_extended=False):
        ...
        self.bus.send(arb_id=arbitration_id, data=data, is_extended=is_extended)
    ...
    def indication(self, wait_window=None, trim_padding=True, first_frame_only=False):
        ...
        while True:
            ...
            if current_time >= end_time:
                ...
                return None
            # Receive frame
            msg = self.bus.recv(timeout=wait_window)
            if msg is not None:
        ...
        if len(frames) == 0:
            ...
            while number_of_frames_left_to_send > 0:
                ...
                while not receiver_is_ready:
                    ...
                    msg = self.bus.recv(timeout=self.N_BS_TIMEOUT)
                    if msg is None:
                        # Quit on timeout
                        return None
